...
def test_freeze_selects_official_final_subset_without_inspecting_answers():
    protocol = AppWorldProtocol.freeze(Package(), model_profile="model", core_planner_hash="core", official_split="test_normal")
...
def test_runner_is_durable_and_reports_measured_usage(tmp_path):
    protocol = AppWorldProtocol.freeze(Package(), model_profile="model", core_planner_hash="core", official_split="test_normal", published_count=2)
    runtime = Runtime()
