...
class DurableBrokerLearningProjection:
    ...
    def project(self, *, environment_id: str, run_id: str, task_id: str, outcome_passed: bool) -> list[tuple[str, Mapping[str, Any]]]:
        ...
        trusted_outcome = get_outcome(run_id) if callable(get_outcome) else None
        if not isinstance(trusted_outcome, Mapping) or not isinstance(trusted_outcome.get("passed"), (bool, int)):
            raise LearningProjectionError("broker history requires a trusted evaluator outcome")
        actual_outcome_passed = bool(trusted_outcome["passed"])
        ...
        for row in joined_rows:
            ...
            record = {
                ...
                "partition": "development", "visibility": "learner", "trustClass": "broker", "trustedOutcome": bool(trusted_outcome), "outcomePassed": actual_outcome_passed,
            }
