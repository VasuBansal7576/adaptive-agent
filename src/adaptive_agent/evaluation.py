...
class EvaluationReport:
    ...
    def final_gate_passed(self) -> bool:
        ...
        if learned.accuracy - baseline.accuracy < 0.05:
            return False
        if baseline.mean_cost_microunits == 0 and learned.mean_cost_microunits > 0:
            return False
        if baseline.p95_latency_seconds == 0 and learned.p95_latency_seconds > 0:
            return False
        return (
            (baseline.mean_cost_microunits == 0 or learned.mean_cost_microunits / baseline.mean_cost_microunits <= 1.10)
