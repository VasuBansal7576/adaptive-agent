...
- A real host bridge request returned run-scoped capability metadata and a
  ...
  `ChildPlanner` callback; learner `rlm.run` requests then execute returned
  bounded code in a fresh Docker kernel with empty capabilities, shared child/
  wall/cancel budget, and attenuated context. Learner-generated bytes transfer
